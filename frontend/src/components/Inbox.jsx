...
const Inbox = () => {
  ...
  // Mocking emails and search text state
  ...
  // Effect to mock fetching emails
  useEffect(() => {
    // Simulate fetching emails (you can replace this with actual fetching logic)
    const fetchedEmails = [
      {
        id: 1,
      ...
      // Add more email mock data if needed
    ];
    setEmails(fetchedEmails);
  }, []);

  // Effect to filter emails based on search text
  useEffect(() => {
  ...
  const handleClick = (email) => {
    ...
    // Handle email selection logic (you can replace with your own logic)
  };
      ...
      <div className="flex flex-col">
        {tempEmails.map((email) => {
          return (
            <div
              key={email.id}
              className="flex gap-2 items-center justify-between border-b h-10 p-2 cursor-pointer hover:shadow-lg"
